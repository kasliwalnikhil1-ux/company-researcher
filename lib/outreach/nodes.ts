...
import type { AbBranch, ActionType, AiRouteOption, ConditionOp, GraphNode, NodeType } from './types';
import { AI_ROUTE_ELSE, CALL_OUTCOMES } from './types';
...
export const NODE_CATALOG: Record<NodeType, NodeMeta> = {
  ...
  refresh_profile: { type: 'refresh_profile', label: 'Refresh profile', group: 'Outreach', description: 'Re-read the full profile when the stored data is old', exits: ['next'], actionType: 'profile_view', defaultConfig: { only_if_stale_days: 90 }, color: 'bg-sky-500' },
  ...
  follow_profile: { type: 'follow_profile', label: 'Follow profile', group: 'Social', description: 'Follow the lead on LinkedIn', exits: ['next'], actionType: 'follow', defaultConfig: {}, color: 'bg-pink-500' },
  ...
  send_voice_note: { type: 'send_voice_note', label: 'Send voice note', group: 'Outreach', description: 'A recorded LinkedIn voice message, one clip per sender', exits: ['next'], actionType: 'message', defaultConfig: { send_always: false }, color: 'bg-blue-500' },
...
export function nodeExits(node: GraphNode): string[] {
  ...
  if (!meta) return [];
  return meta.dynamicExits ? meta.dynamicExits(node) : meta.exits;
}
