...
import { ArrowLeft, Building2, Compass, FileSpreadsheet, MessageSquare, Search, ShieldAlert, ThumbsUp, Users } from 'lucide-react';
    ...
    <div>
      ...
      {!canWrite ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px,1fr] gap-4">
          <div className="flex lg:flex-col gap-2 overflow-x-auto pb-1 lg:pb-0" role="tablist" aria-label="Import source" aria-orientation="vertical">
            {SOURCES.map((s) => (
              <button key={s.id} type="button" role="tab" id={`import-tab-${s.id}`} aria-controls="import-panel" aria-selected={source === s.id} onClick={() => setSource(s.id)}
                className={cn('flex-1 lg:flex-none text-left rounded-xl border p-3 transition-colors min-w-[200px] focus:outline-none focus:ring-2 focus:ring-indigo-500', source === s.id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white hover:bg-gray-50')}>
