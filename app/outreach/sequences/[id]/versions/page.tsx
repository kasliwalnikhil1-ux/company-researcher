...
import { ArrowLeft, Code2, Eye, History, LayoutGrid, RotateCcw } from 'lucide-react';
...
export default function SequenceVersionsPage() {
  ...
  const doRestore = async () => {
    ...
    setBusy(true);
    try {
      const v = await rpc<number>('restore_sequence_version', { p_id: id, p_version: restore.version });
      qc.invalidateQueries({ queryKey: qk.sequence(id) });
      qc.invalidateQueries({ queryKey: qk.sequenceVersions(id) });
      qc.invalidateQueries({ queryKey: qk.sequences(seq.data.workspace_id) });
      toast.show(`Restored version ${restore.version} as version ${v}`);
      setRestore(null);
  ...
  return (
    <div>
      ...
      <PageHeader title={<span className="flex items-center gap-2"><History className="w-6 h-6 text-gray-400" /> Version history</span>} subtitle={<>{seq.data.name} · current version v{seq.data.head_version}. Every save that changes the graph creates a version; restoring copies a version to the head as a new version.</>} />
      {versions.error ? <ErrorBox message={parseError(versions.error).message} /> : list.length === 0 ? <EmptyState title="No versions yet" description="Save the sequence to create the first version." /> : (
        <Table>
          <thead><tr><Th>Version</Th><Th>Saved</Th><Th>By</Th><Th className="text-right">Steps</Th><Th>Compared to current</Th><Th className="text-right">Actions</Th></tr></thead>
          <tbody>
            {list.map((v) => {
              ...
              return (
                <tr key={v.version} className="hover:bg-gray-50">
                  <Td><span className="font-medium text-gray-900">v{v.version}</span>{isHead && <Badge tone="indigo" className="ml-2">current</Badge>}</Td>
      ...
      <ConfirmModal open={!!restore} title={`Restore version ${restore?.version}`} confirmLabel="Restore" busy={busy} onClose={() => setRestore(null)} onConfirm={doRestore}
        body={<><p>The graph of version {restore?.version} becomes the new head (saved as v{seq.data.head_version + 1}). Pool, settings and name are unchanged.</p>{seq.data.status === 'active' && <p className="text-xs text-amber-700">The sequence is active: the restored graph must pass strict validation, and live enrollments continue from their current step ids.</p>}</>} />
      {toast.node}
    </div>
  );
}
