...
/**
 * POST /api/data-pipelines/rerun-profile
 *
 * Fetch investors from the investors table filtered by type and updated_at range.
 * Only returns investors that have a non-null deep_research column (prerequisite for Step 3).
 *
 * Body: { type?: 'firm' | 'person', updatedAtFrom?: string, updatedAtTo?: string }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, updatedAtFrom, updatedAtTo } = body;

    // Fetch investors filtered by type and updated_at, must have deep_research
    let query = supabase
      .from('investors')
      .select('id, name, type, domain, linkedin_url, updated_at')
      .not('deep_research', 'is', null)
    ...
    console.error('rerun-profile error:', error);
