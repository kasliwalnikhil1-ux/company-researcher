...
/**
 * GET /api/linkedin-conversations/automations
 * Proxy to GetSales: GET /flows/api/flows
 * Returns the list of automations (flows) for filter UIs.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const gsUrl = `${GETSALES_BASE}/flows/api/flows?${params.toString()}`;
    ...
    if (!gsRes.ok) {
      ...
      console.error('[automations] GetSales error:', gsRes.status, errText);
