-- Add email_verified (from the Gmail-Compose verified-email pipeline) to
-- search_investors so the app can show whether an investor's email is verified.
-- Gated by personalization like email itself (NULL unless the caller has the
-- investor in their list). Return type CHANGED, so the old function must be
-- dropped and grants re-applied (all in one transaction below).
...
base as (
  select
    ...
    case when ip.id is not null then i.email_verified end as email_verified,
