-- Two search fixes in search_investors (applied to ktwqkvjuzsunssudqnrt on 2026-08-04):
--
-- 1. Comma-separated multi-term search: p_query is split on commas and a row matches
--    if ANY term matches its name/domain/linkedin_url (e.g. "Leon Yao, Marguerite Colin"
--    finds both people). Previously the whole string was one ILIKE pattern, so comma
--    lists matched nothing.
--
-- 2. NULL active now behaves as inactive instead of vanishing entirely: with the old
--    `i.active = p_active`, rows with active IS NULL matched NEITHER the Active nor the
--    Inactive filter (comparison yields NULL) — they only appeared with no filter at all.
--    Rows added via the MCP connector don't set active, so 483 rows were invisible in the
--    app's default Active view. Per user decision, unknown = inactive:
--    `coalesce(i.active, false)` hides them from the Active view but shows them under the
--    Inactive filter. All 483 remain NULL; to surface one in the Active view, explicitly
--    set active = true.
--    Companion DDL (applied): ALTER TABLE public.investors ALTER COLUMN active SET DEFAULT true;
--    so future inserts that omit the column are visible by default.
--
-- Return type unchanged from search_investors_add_updated_at.sql, so CREATE OR REPLACE
-- is enough (no DROP / re-GRANT needed).
...
paging as (
  select
    ...
    case
      ...
      when p_mode = 'global' then least(coalesce(p_limit, 10), 10)
...
base as (
  select
    ...
    case when i.type = 'person' then f.name end as associated_firm_name,
  ...
  left join public.investor_affiliations af
    on i.type = 'person'
   and af.person_id = i.id

  left join public.investors f
    on f.id = af.firm_id
  ...
  where
    ...
    -- query: any comma-separated term matches OR (people search + firm match -> affiliated people)
    and (
      p_query is null
      or exists (
        select 1
      ...
      or (
        p_type = 'person'
        and i.type = 'person'
        and af.firm_id in (select mf.id from matching_firms mf)
      )
    )
),
numbered as (
