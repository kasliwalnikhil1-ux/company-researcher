-- Fix: a firm's Contacts tab showed at most 10 people even when the firm had
-- more (e.g. Entrepreneurs First: "21 contacts available" but only 10 listed).
-- The paging CTE clamped EVERY global-mode request to 10 rows, including the
-- firm-roster fetch (p_investor_id + p_type='person'), where the app requests
-- up to 100 contacts for paid users (InvestorDetailsDrawer contactsLimit).
--
-- Fix: the global-mode 10-row clamp (an anti-scrape guard for the global search
-- table) now applies only when p_investor_id IS NULL. A query scoped to one
-- investor's roster honors p_limit. Free-plan behavior unchanged (still 10 max,
-- offset 0; the app requests 5 there anyway).
--
-- Only the paging CTE changed vs search_investors_dedupe_multi_affiliation.sql.
-- Return type unchanged, so CREATE OR REPLACE is enough (no DROP / re-GRANT).
