-- Contacts must be ACTIVE investors only (user decision 2026-08-04):
-- 1. associated_people_count ("X contacts available" in the drawer) previously counted
--    ALL affiliated people; now it counts only people with coalesce(active, false) = true,
--    so inactive/unknown people don't inflate the count.
-- 2. The firm-roster fetch (Contacts tab) excludes them too: fetchPeopleAtFirm in
--    hooks/useInvestorSearch.ts now passes p_active = true (no SQL change needed for
--    that part; the existing p_active filter applies).
--
-- Everything else identical to search_investors_firm_roster_limit.sql (comma-term query,
-- coalesce(active,false) filter semantics, single display affiliation, roster paging).
