-- Fix: people with multiple firm affiliations appeared once PER affiliation in
-- search results (e.g. Harry B. Shimp @ MCM Capital + Entrepreneurs First showed
-- twice in the investors list and in firm Contacts). The old
-- `left join investor_affiliations af` fanned out one row per affiliation.
--
-- Fix: replace the fan-out join with a `left join lateral ... limit 1` that picks
-- exactly ONE display affiliation per person:
--   - when browsing a firm's contacts (p_investor_id + p_type='person'), the
--     queried firm is preferred, so the card shows the firm you're looking at;
--   - otherwise the alphabetically-first firm, deterministically.
-- The comma-query branch that matched people via their firm's name/domain now
-- uses EXISTS over all affiliations (not just the picked one), so query matching
-- behavior is unchanged.
--
-- Multi-affiliations became common on 2026-08-04 when the MCP connector's
-- update_investor gained firm_domains multi-affiliation support; the fan-out had
-- been latent since the join was introduced.
--
-- Return type unchanged from search_investors_comma_separated_query.sql, so
