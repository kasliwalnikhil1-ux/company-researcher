-- Add updated_at (investors row timestamp, maintained by trg_investors_updated_at) to
-- search_investors so the app can invalidate client-side caches (contacts, deep research)
-- when an investor row changes. Applied to ktwqkvjuzsunssudqnrt on 2026-08-04.
-- NOTE: return type changed, so the old function must be dropped first and grants re-applied:
-- DROP FUNCTION public.search_investors(uuid,text,text,boolean,text[],text[],text,text,text[],numeric,numeric,numeric,numeric,text[],text[],text[],boolean,text[],text[],text[],text[],text[],text[],text[],boolean[],text,integer,integer);
-- ... then run the CREATE below, then:
-- GRANT EXECUTE ON FUNCTION public.search_investors(uuid,text,text,boolean,text[],text[],text,text,text[],numeric,numeric,numeric,numeric,text[],text[],text[],boolean,text[],text[],text[],text[],text[],text[],text[],boolean[],text,integer,integer) TO anon, authenticated, service_role;
...
matching_firms as (
  ...
  where inv.type = 'firm'
    ...
    and (
      inv.name ilike '%' || p_query || '%'
      or inv.domain ilike '%' || p_query || '%'
    )
),
base as (
  select
  ...
  where
    ...
    -- query: normal match OR (people search + firm match -> affiliated people)
    and (
      p_query is null
      or i.name ilike '%' || p_query || '%'
      or i.domain ilike '%' || p_query || '%'
      or i.linkedin_url ilike '%' || p_query || '%'
      or (
        p_type = 'person'
        and i.type = 'person'
