---
name: capitalxai
description: Manage the CapitalxAI investor database via its MCP connector. Use when the user wants to add or update investor firms/people, link people to firms, or record and update funding rounds, and the CapitalxAI connector tools (find_investors, add_investor, update_investor, list_fundings, get_funding, add_funding, update_funding) are available.
---
...
Write tools (add/update investor, add/update funding) only appear for **admin accounts** — the same accounts allowed to use Data Pipelines and "Add New Funding" inside the app. If a user only sees the read tools, tell them their account does not have admin access; changes must be made by an administrator.
