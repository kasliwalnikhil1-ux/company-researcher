...
export function rulesDoc(ctx: Ctx): string {
  ...
  return [
    ...
    "2. **One approval, one send.** A token works once, for one lead, for 10 minutes. \"Looks good, do the rest\" approves nothing else. Never batch.",
...
export function registerPrompts(server: McpServer, ctx: Ctx): void {
  ...
  server.registerPrompt("reply_triage", { title: "Reply triage and send", description: "Read a thread, categorise, pause follow-ups, draft, send on approval — one lead at a time", argsSchema: { lead: z.string().optional().describe("Lead email or name; empty = the next unanswered positive reply") } },
    ({ lead }) => prompt(`${RULES_NOTE}\n\nReply triage${lead ? ` for ${lead}` : ""}:\n1. ${lead ? `list_replies(search:"${lead.slice(0, 30)}")` : "list_replies(uncategorised:true) and list_replies(category:\"Interested\") — take the most valuable unanswered one"} → get_reply(campaign_id, lead_id).\n2. If get_reply says automated (OOO / bounce / auto-responder / unsubscribe): update_lead_category accordingly (Do Not Contact + pause_lead:true for unsubscribes), tell me, and move to the next. No reply.\n3. Otherwise, in ONE message: who they are, what they wrote (quoted), your category call, and a draft reply — short, answers their actual question, one clear next step, no signature (the mailbox signature is appended), no {{variables}}.\n4. I edit in chat until it reads right. When I approve the final text: update_lead_category (+ pause_lead:true so the sequence stops), then reply_to_thread(body = exactly that text). Show me the effect_summary verbatim; on my yes call it again with the confirmation_token.\n5. Confirm: sent, log id, sends remaining. Then offer the next thread. Each thread needs its own approval — never carry a yes over.`));
