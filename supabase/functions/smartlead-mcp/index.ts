...
"Any pending replies?" / "what needs answering?": read the page of replies (list_replies, then get_reply for each human one), judge uncategorised ones yourself, and in the SAME turn show one numbered table — who (lead, company, campaign, mailbox, date) · their exact words (their_words / they_wrote verbatim, quoted) · contact they shared (contacts.mentioned_in_reply — emails/numbers they wrote, with whose they are; a referral also gets an email draft to that person) · draft reply · next action — then automated/not-interested threads as one line each with the category you propose. Never stop at a summary asking whether to draft. Drafting sends nothing; each accepted draft still goes through its own reply_to_thread summary and its own yes.
