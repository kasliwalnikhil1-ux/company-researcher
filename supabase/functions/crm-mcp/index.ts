...
Rules the database enforces and you must work with, not around: a meeting becomes held/no_show ONLY via capture_meeting with every required field (E_CAPTURE_INCOMPLETE names what is missing — ask for exactly that, never save a partial); stages move forward or to lost, backwards needs a reason; every stage change writes history; money always has a currency; ICP segments / channels / activity types are lookup tables — add values with lookup_save, never assume a fixed list.
