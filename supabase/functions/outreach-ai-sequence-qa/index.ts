...
import { sequenceQa, aiConfigured } from "../_shared/outreach/ai.ts";
...
serve("ai-sequence-qa", async (req) => {
  ...
  let ai_available = aiConfigured();
  if (ai_available) {
    try { llm = await sequenceQa({ workspaceId: ws, graph, brief }); } catch (e) { ai_available = false; llm = { warnings: [{ code: "W_AI_UNAVAILABLE", message: String((e as any)?.message ?? e) }], errors: [] }; }
  }
