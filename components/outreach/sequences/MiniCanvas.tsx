...
import { deriveEdges, deriveNodes, edgeTypes, nodeTypes, type OutreachRFEdge, type OutreachRFNode } from './Canvas';
...
export default function MiniCanvas({ graph, lookup = {}, className }: { graph: Graph; lookup?: Lookup; className?: string }) {
  ...
  const edges = useMemo<OutreachRFEdge[]>(() => deriveEdges(graph, true), [graph]);
  return (
