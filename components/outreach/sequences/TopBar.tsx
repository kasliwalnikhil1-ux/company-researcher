...
import { AlertTriangle, Archive, ArrowLeft, Check, ChevronDown, CloudOff, HelpCircle, History, Loader2, Maximize2, Pause, Pencil, Play, RotateCcw, Save, Settings2, Shuffle, TrendingUp, Undo2, UploadCloud, UserPlus, UserX, Zap } from 'lucide-react';
...
function DraftStatusText({ i, dirty, publishMode, readOnly }: { i: DraftIndicator; dirty: boolean; publishMode: boolean; readOnly: boolean }) {
  ...
  const box = 'text-xs inline-flex items-center gap-1.5 md:w-[24rem] min-w-0 h-5 whitespace-nowrap';
...
export default function TopBar(p: Props) {
  const { sequence, draft, dirty, saving, version, readOnly, canManage, publishMode, modeKnown, canDiscard, indicator, senders, clients, inflight, failedCount, onChange, onSave, onPublish, onDiscard, onPoolApplied, onStatus, onWhy, onAutoEnrol, onFailed, onFit, onNavigate } = p;
    ...
    <div className="bg-white border-b border-gray-200">
      <div className="flex flex-wrap items-center gap-2 px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-1.5 flex-wrap">
          ...
          {readOnly && sequence.status !== 'archived' && <Button variant="ghost" size="sm" onClick={() => onNavigate(`${base}/enroll`)} title="Leads in this sequence"><UserPlus className="w-4 h-4" /><span className="hidden sm:inline">Leads</span></Button>}
          {!readOnly && (
